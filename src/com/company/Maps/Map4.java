...
public class Map4 implements Map
{
    ...
    Force[] forces;
    int currentLevel;
    final int mapDifficultyConstant = 4; // TODO: Find good constants to differentiate between maps
    final int ID = 4;
    ...
    public Map4()
    {
        ...
        try
        {
            bq = new Image(new FileInputStream("src\\com\\company\\resources\\maps\\4.jpg"));
        ...
        for (int i = 0; i < forces.length-1; i++)
        {
            ...
            if (forces[i].isDownwards())
            {
                ...
                forces[i].setPower(-((mapDifficultyConstant + randomWithRange(1,10)) /1.0 ));
            }
            else
            {
                forces[i].setPower(+((mapDifficultyConstant + randomWithRange(1,10)) /1.0 ) );
        ...
        forces[9] = new Force(mapDifficultyConstant/50.0,true);
